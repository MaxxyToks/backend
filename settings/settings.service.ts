...
export class SettingsService {
  ...
  getSettings(): SettingsIface {
    ...
    const redisUrl = this.createRedisConnectionUrl(redisHost, redisPort, redisPassword);
    ...
    return {
      ...
      keys: {
        ...
        dexTools: getOrThrow('DEX_TOOLS_KEY'),
      },
      kms: {
      ...
      across: {
        apiUrl: getOrThrow('ACROSS_API_URL'),
      },
      redis: {
        host: redisHost,
        port: redisPort,
      ...
      oneinch: {
        apiUrl: getOrThrow('ONE_INCH_API_URL'),
        apiKey: getOrThrow('ONE_INCH_API_KEY'),
      },
      qdrant: {
        url: getEnv('QDRANT_URL'),
      },
      dextools: {
        url: getOrThrow('DEXTOOLS_URL'),
      },
      moralis: {
        apiKey: getOrThrow('MORALIS_API_KEY'),
      },
      alchemy: {
        apiKey: getOrThrow('ALCHEMY_API_KEY'),
      },
      helius: {
        apiKey: getOrThrow('HELIUS_API_KEY'),
      },
      solana: {
        rpc: getOrThrow('HELIUM_RPC_URL'),
      },
      feeConfig: {
        swapFee: getOrThrow('SWAP_FEE'),
        transferFee: getOrThrow('TRANSFER_FEE'),
        bridgeFee: getOrThrow('BRIDGE_FEE'),
        evmTreasury: getOrThrow('EVM_TREASURY'),
        solanaTreasury: getOrThrow('SOLANA_TREASURY'),
      },
    };
  }
